...
- us_china_trade: REPURPOSED AS US-JAPAN ALLIANCE & TRADE — object with sub-blocks (NO REPETITION across sub-blocks):
  - tariff_tracker: object with headline_auto_rate (Japan's headline auto rate — default "15% (incl. MFN, 2025 agreement)"), section_232_rates (object with ONLY steel and aluminum, e.g. {{"steel":"50%","aluminum":"50%"}} — do NOT put autos here; Japan's autos are covered by the 15% 2025-agreement rate, not the old 25% Section 232 rate), section_122_surcharge (string or null — EXPIRED July 24, 2026; default null, i.e. do NOT report a Section 122 surcharge unless today's articles announce a NEW one), section_301 (string or null — default "12.5% forced-labor tariff in force (Jul 24, 2026); excess-capacity investigation ongoing"; update only from today's articles; the forced-labor tariff is IN FORCE, the excess-capacity probe is ONGOING — do NOT label either "proposed"), investment_framework (string or null — the $550 billion US-Japan strategic investment framework; default "$550B strategic investment framework (2025 agreement)"; update specifics only from today's articles, never invent figures), trade_deal_status (1 phrase — default "2025 US-Japan agreement in force"), last_change (string), next_trigger (string or null — default null; fill only if today's articles name a dated upcoming trigger). Use the ALLIANCE & TRADE BASELINES above as default; only change if today's articles report new action.
...
- public_sentiment: Japan cabinet approval & party-support polling block. SAME-POLL-DATE RULE (applies WITHIN each poll object): every figure inside a single poll object must come from ONE survey by ONE pollster for ONE date range — never blend pollsters or dates inside one object. Object with:
  - approval_polls: array of 1-3 poll objects, at most ONE per pollster, so readers can see how the houses differ. SOURCE: build these primarily from the CABINET-APPROVAL POLL REPORTS block above (per-pollster, last 30 days) — that block exists specifically so this table is populated even between the ~monthly poll releases. Include the most recent poll from each DIFFERENT recognized Japanese pollster (order the most authoritative/most recent first — that first entry is the "primary" poll), each labeled with its real survey date even if it's a few weeks old. HARD RULES:
    ...
    * Empty array if no qualifying Japanese-pollster poll appears today (or carry forward the most recent cited one(s), each labeled with its original date). Do NOT invent a pollster or number to fill the array — 0 clean polls is better than 1 fabricated or vague one.

...
- public_sentiment: Japan cabinet approval & party-support polling block. SAME-POLL-DATE RULE (applies WITHIN each poll object): every figure inside a single poll object must come from ONE survey by ONE pollster for ONE date range — never blend pollsters or dates inside one object. Object with:
  - approval_polls: array of 1-3 poll objects, at most ONE per pollster, so readers can see how the houses differ. Include the most recent poll from each DIFFERENT pollster that appears in today's articles (order the most authoritative/most recent first — that first entry is the "primary" poll). Each object: pollster (NHK / Jiji / Yomiuri / Asahi / Kyodo), poll_date (date range string, e.g. "Jun 13-15"), cabinet_approval (e.g. "42%"), cabinet_disapproval (e.g. "38%" or null if that poll doesn't report it), approval_change (vs prior same-pollster poll, e.g. "↓3" or null). Empty array if no poll appears in today's articles (or carry forward the most recent cited poll(s), each clearly labeled with its original date). Do NOT invent a pollster or a number to fill the array — 1 real poll is better than 3 with any fabricated.
